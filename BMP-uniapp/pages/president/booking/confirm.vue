<template>
  <view class="confirm-order bg-surface min-h-screen">
    ...
    <scroll-view scroll-y class="main-content" :style="{ height: scrollHeight }">
      ...
      <!-- Member Info (President specific) -->
      <view v-if="bookingInfo.memberName" class="mb-8 bg-white rounded-2xl p-5 border border-neutral-100 card-glow flex flex-row items-center gap-4">
        <view class="w-12 h-12 rounded-full bg-primary-10 flex items-center justify-center shrink-0">
          <uni-icons type="person-filled" size="24" color="#ff6600"></uni-icons>
        </view>
        <view>
          <text class="text-[10px] text-secondary font-bold uppercase tracking-widest mb-1 block">预约会员 MEMBER</text>
          <text class="font-bold text-on-surface">{{ bookingInfo.memberName }}</text>
        </view>
      </view>
      ...
      <view class="mb-10">
        <view class="flex flex-row items-center justify-between mb-4 px-1">
          <text class="text-[11px] font-black uppercase tracking-[0.15em] text-secondary/70">确认结算方式</text>
          <text class="text-[10px] text-primary font-bold">管理权限支付</text>
        </view>
        <view class="space-y-3">
          ...
          >
            <view class="flex flex-row items-center gap-4">
              ...
              <view>
                <text class="font-bold text-on-surface text-base block">会员余额代扣</text>
                <view class="flex flex-row items-center">
                  <text class="text-[11px] text-secondary font-medium">会员可用: </text>
                  <text class="text-[11px] text-primary font-bold ml-1">¥{{ bookingInfo.memberBalance || '2,480.00' }}</text>
                </view>
              </view>
            </view>
      ...
      <view class="bg-white rounded-2xl p-6 card-glow border border-neutral-100">
        ...
        <view class="space-y-4">
          ...
          <view class="flex flex-row justify-between items-center text-primary">
            <view class="flex flex-row items-center gap-2">
              <text class="text-sm font-medium">后台调价 / 折扣</text>
              <view class="bg-primary-10 px-2 py-0.5 rounded ml-1">
                <text class="text-[10px] italic font-black uppercase text-primary">管理端优惠</text>
              </view>
            </view>
      ...
      <!-- Terms Agreement (Disabled for president as they override policy) -->
      <view class="flex flex-row items-start gap-3 px-2 mt-8 mb-4">
        ...
        <view class="flex-1">
          <text class="text-[13px] text-secondary leading-snug font-medium">
            管理员确认：
            <text class="text-primary font-bold">该笔预约由协会会长审核并担保。</text>
          </text>
        </view>
      </view>
      </view>
    ...
    <view class="fixed-bottom bg-white-80 backdrop-blur-xl border-t border-neutral-100 px-6 pt-4 pb-safe">
      <view class="flex flex-row items-center gap-5">
        ...
        <button 
          class="flex-1 bg-primary text-white rounded-2xl py-3 energy-shadow flex flex-col items-center justify-center overflow-hidden relative active-scale"
          @tap="handlePayment"
        >
          <view class="absolute inset-0 bg-white-10 opacity-0 active-opacity-20 transition-opacity"></view>
          <text class="font-headline font-black text-base uppercase tracking-widest leading-none block">确认并结算</text>
          <text class="text-[9px] font-bold opacity-80 mt-1 uppercase tracking-tighter block">ADMIN CHECKOUT • ¥{{ bookingInfo.payableAmount.toFixed(2) }}</text>
        </button>
      </view>
    </view>
...
const handlePayment = () => {
  uni.showLoading({ title: '后台结算中...' })
  setTimeout(() => {
    uni.hideLoading()
    uni.showToast({ title: '确认成功', icon: 'success' })
    setTimeout(() => {
      uni.reLaunch({ url: '/pages/president/booking/list' })
    }, 1500)
  }, 1500)
}
</script>
