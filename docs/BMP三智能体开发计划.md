...
| 里程碑 | 对应阶段 | 可验证成果 | 状态 |
| --- | --- | --- | --- |
| M1 Agent 服务可运行 | 阶段 1 | FastAPI + LangGraph 最小闭环 | 已达成（2026-07-18） |
| M2 Java Tool 安全打通 | 阶段 2 | 鉴权后的只读 Tool 调用与六道防线 | 已达成（2026-07-21） |
| M3 自然语言预订闭环 | 阶段 3 | 查询、推荐、确认、幂等创建 | 已达成（2026-07-22） |
| M4 经营分析可用 | 阶段 4 | 权限范围内的可信指标分析 | 已达成（2026-07-22） |
| M5 RAG 客服可用 | 阶段 5 | 带引用回答、拒答和转人工 | 已达成（2026-07-22） |
| M6 多端接入完成 | 阶段 6 | Web 和 UniApp 主流程 | 框架达成（2026-07-23，待端到端验证） |
| M7 具备生产条件 | 阶段 7 | 监控、安全、评估、灰度和回滚 | 待实施 |
