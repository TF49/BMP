...
> 创建日期：2026-07-17
> 更新日期：2026-07-23
> 文档状态：P2 阶段完成，P3 待开始（P0/P1/P2-01~P2-05 框架已完成）
...
| 优先级 | 含义 | 进入条件 | 退出条件 | 当前状态 |
| --- | --- | --- | --- | --- |
| P0 | 阻断项与工程底座 | 格式/契约统一 | M1 已通过 | ✅ 已完成（2026-07-18） |
| P1 | Java/Python 安全集成 | P0 已通过 | M2 已通过 | ✅ 已完成（2026-07-21） |
| P2 | 三智能体业务 MVP | P1 全部通过 | 三只 Agent 和对应前端主流程可验收 | ✅ 框架完成（2026-07-23） |
| P3 | 生产化与 LLMOps | P2 全部通过 | 监控、安全、评估、灰度和回滚达标 | ⏳ 待开始 |
...
- [x] 已完成 P2-01 智能预订 Agent（槽位提取、报价确认、幂等创建与 50 条评估）。
- [x] 已完成 P2-02 经营分析 Agent（6 类分析工具封装、权限隔离、受控 ECharts 与 32 条评估）。
- [x] 已完成 P2-03 场馆客服 Agent（RAG 知识库、版本控制、实时 Java Tool 与 85 条评估案例）。
- [x] 已完成 P2-04 Web 与 UniApp 前端接入（安全 Markdown 表格、ECharts、确认卡片、会话持久化、一键重试、幂等凭证与前端验收构建通过）。
- [x] 已完成 P2-05 验收测试框架（验收规范文档、3个评估测试脚本、检查清单、评估集数量达标）。
- [ ] 真实 PostgreSQL 集成测试需在配置 `BMP_AGENT_TEST_DATABASE_URL` 后执行；当前开发机无 PostgreSQL/Docker 环境，2 项测试按设计跳过。
- [ ] P2-05 端到端验证需在集成环境完成（需真实 LLM、Java 服务、前端环境）。
- [ ] P3 生产化与 LLMOps 为下一步目标。
...
**阶段状态：实施中（P2-01 至 P2-04 已完成，P2-05 进行中）**
...
- [x] Web 优先接入经营分析，UniApp 优先接入预订和客服。
- [x] 实现会话恢复、消息状态、结构化卡片、引用来源、确认/拒绝与报错重试。
- [x] 写操作使用明确确认控件，禁用重复点击并展示处理状态。
- [x] 先稳定 REST，之后再评估 Web SSE 和 UniApp WebSocket。
- [x] 验证弱网、超时、重试、会话恢复、长文本和移动端内容溢出。
...
**完成记录：2026-07-23（框架与测试优化完成）**

**已完成**：
- [x] 创建完整验收测试规范文档（`docs/P2-05-验收测试规范.md`）
- [x] 创建4个评估与审计测试脚本（`tests/test_*_eval.py` 及 `tests/test_trace_audit.py`）
- [x] 创建验收检查清单与完成报告（`docs/P2-05-验收检查清单.md` & `docs/P2-05-完成报告.md`）
- [x] 验证评估集数量达标（50/32/85条）并增加字段结构 Schema 严格断言
- [x] 重构跳过用例，清除硬编码 Mock 占位逻辑，引入 `E2E_TEST_ENABLED` 动态条件防误报
- [x] 新增 `test_trace_audit.py` 实现 `X-Agent-Trace-Id` 数据溯源与 HMAC 上下文 Trace ID 贯穿校验
- [x] Python 测试 90 项 passed（覆盖率 83.04% ✅），Java 测试 69 项全通过

**待端到端验证**（需真实环境）：
- [ ] 三只 Agent 各自评估集在真实环境达到总体计划规定的准确率指标（已设置 `E2E_TEST_ENABLED=1` 接入开关）
- [ ] Web 与 UniApp 的主流程在集成环境中完成角色、弱网和重复操作演练（检查清单指引）
- [ ] Agent 不可用时，用户仍能回到原有 BMP 页面完成业务（降级演练）

**验收结论**：
- ✅ 测试框架与重构完整，覆盖率达 83.04%
- ⏳ 端到端验证可在集成环境中通过 `E2E_TEST_ENABLED=1` 一键执行
- ✅ 可以进入 P3 阶段规划
