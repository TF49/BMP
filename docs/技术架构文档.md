...
| 层级 | 技术 | 版本要求 |
| --- | --- | --- |
| Agent API | Python 3.12、FastAPI、Pydantic 2 | Python >= 3.12 |
| Agent 编排 | LangGraph | 最新稳定版 |
| 模型适配 | OpenAI-compatible 接口 | - |
...
3. **上下文生成**：Spring Boot 生成短期签名的 Agent 上下文，调用 FastAPI
4. **Agent 路由**：FastAPI 根据显式的 `agentType` 进入对应子图
...
- 统一返回 `code`、`message`、`data` 和 `traceId`
