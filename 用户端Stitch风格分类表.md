...
- 登录、注册、找回密码这类入口页保留当前版本，不再纳入后续 Stitch 改版范围
- 目前主要剩余的旧风格页面集中在少量设置尾页和外围功能页

最大的结构性问题不是单页不好看，而是：

- 多条核心业务链路已经完成 Stitch 化
- 场馆链路也已经补齐到“列表页 + 详情页 + 预约页”的连续新版体验
- 风格断层已经从“主链路断层”进一步收敛为“少量外围功能断层”
...
| 页面 | 路径 | 分类判断 | 原因 |
| --- | --- | --- | --- |
| 少量设置尾页 | `pages/settings/*` | 非 Stitch | 用户端帮助链路已收敛到 `pages/settings/help-user.vue`，原 `pages/settings/help.vue` 继续保留给会长端入口使用；剩余设置页只需按实际业务继续取舍 |
...
1. `pages/profile/member.vue`
2. `pages/notice/index.vue`

原因：

- 场馆链路已经补齐后，最明显的断层已经转移到少量高频外围页面
- 用户会在会员中心和通知中心这些日常高频节点感受到是否还是同一套产品
...
- 用户中心是高频使用区域
- 当前主链路已经统一，剩余高频页面可以继续按同一套安全感、留白和卡片语言做微调

### 第三优先级：按实际业务再决定是否继续扩展
...
- 当前已删除无业务的反馈页与 FAQ 页，后续不必为了补分类而继续做空页面
- 更合适的做法是按真实业务存在与否决定是否继续扩展页面骨架
...
当前 `BMP-uniapp` 用户端已经完成多条核心业务链路的 Stitch 化，课程、赛事、器材、充值、穿线与场馆主链路都已有新版页面承接，搜索首页、搜索结果页与穿线查询页也已进入新版体系；登录、注册、找回密码保留当前版本不再继续改动，且无业务的反馈页与 FAQ 页已移除，接下来只需要按真实存在的高频页面继续收尾即可。 
